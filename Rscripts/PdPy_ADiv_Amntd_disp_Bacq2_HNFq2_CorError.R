...
##### Loading nulls ###########################################################################
###############################################################################################
Bac_Amntd_null <- read.table(file = "D:/Research/PdPy_Div_Results/nulls_PR2_2/Bac_Amntd_null.csv", sep = ",", 
                             header = TRUE, stringsAsFactors = FALSE, fill = TRUE)
Bac_Ampd_null <- read.table(file = "D:/Research/PdPy_Div_Results/nulls_PR2_2/Bac_Ampd_null.csv", sep = ",", 
                            header = TRUE, stringsAsFactors = FALSE, fill = TRUE)
Bac_Bmntd_null <- read.table(file = "D:/Research/PdPy_Div_Results/nulls_PR2_2/Bac_Bmntd_null.csv", sep = ",", 
                             header = TRUE, stringsAsFactors = FALSE, fill = TRUE)
Bac_Bmpd_null <- read.table(file = "D:/Research/PdPy_Div_Results/nulls_PR2_2/Bac_Bmpd_null.csv", sep = ",", 
                            header = TRUE, stringsAsFactors = FALSE, fill = TRUE)
Bac_Chao_null <- read.table(file = "D:/Research/PdPy_Div_Result/nulls_PR2_2s/Bac_Chao_null.csv", sep = ",", 
                            header = TRUE, stringsAsFactors = FALSE, fill = TRUE)
...
HNF_Ampd_null <- read.table(file = "D:/Research/PdPy_Div_Results/nulls_PR2_2/HNF_Ampd_null_PR2.csv", sep = ",", 
                            header = TRUE, stringsAsFactors = FALSE, fill = TRUE)
HNF_Bmntd_null <- read.table(file = "D:/Research/PdPy_Div_Results/nulls_PR2_2/HNF_Bmntd_null_PR2.csv", sep = ",", 
                             header = TRUE, stringsAsFactors = FALSE, fill = TRUE)
HNF_Bmpd_null <- read.table(file = "D:/Research/PdPy_Div_Results/nulls_PR2_2/HNF_Bmpd_null_PR2.csv", sep = ",", 
                            header = TRUE, stringsAsFactors = FALSE, fill = TRUE)
...
#         strip.text.y = element_text(angle = 45, color = "black", size = 14))
p_Adiv_AAssemb_zoompairs <- HNF_Bac_A %>%
  ggpairs(columns = c("ln.Bac_q2", "ln.HNF_q2", "Bac_Amntd_select", "HNF_Amntd_select"),
          columnLabels = c("Bacteria\nSimpson\ndiversity", "HNF\nSimpson\ndiversity",
                           "Bacteria\naNTI", "HNF\naNTI"),
          #mapping = ggplot2::aes(colour = Cruise),
          upper = list(continuous = cor_fun),
...
ggsave(p_Adiv_AAssemb_zoompairs, file = "D:/Research/PdPy_Div_Results/Figs/PR2_2/p_Adiv_AAssemb_zoompairs_2.png",
       dpi = 600, width = 34, height = 28, units = "cm")
...
force.labs <- c("Bacteria \U03B1NTI", "HNF \U03B1NTI", "Dispersal force on Bacteria", "Dispersal force on HNF")
...
gam0 <- gam(ln.HNF_q2 ~ ln.Bac_q2, data = HNF_Bac_A)
gam1 <- gam(ln.HNF_q2 ~ s(ln.Bac_q2), data = HNF_Bac_A)
anova(gam0, gam1, test = "F")

gam_select <- gam(ln.HNF_q2 ~ ln.Bac_q2*Bac_Amntd_select + ln.Bac_q2*HNF_Amntd_select + Bac_disp + HNF_disp, data = HNF_Bac_A)
summary(gam_select)
lme_select <- lme(ln.HNF_q2 ~ ln.Bac_q2*Bac_Amntd_select + ln.Bac_q2*HNF_Amntd_select + Bac_disp + HNF_disp, 
                  random = ( ~1 | Cruise), data = HNF_Bac_A)
summary(lme_select)
### plotting (without selection)
p_HNFq2_Bacq2 <- HNF_Bac_A %>% 
  select(ln.Bac_q2, ln.HNF_q2, Bac_Amntd_select, HNF_Amntd_select) %>%
  ggplot(aes(x = ln.Bac_q2, y = ln.HNF_q2)) + 
  geom_point(size = 3) + 
...
p_HNFq2_Bacq2
ggsave(p_HNFq2_Bacq2, file = "D:/Research/PdPy_Div_Results/Figs/PR2_2/p_ADiv_HNFq2_Bacq2.png",
       dpi = 600, width = 34, height = 28, units = "cm")
##### without selection #####

##### add selection and dispersal #####
### linear model
HNFq2_Bacq2_0 <- glm(ln.HNF_q2 ~ ln.Bac_q2, data = HNF_Bac_A)
HNFq2_Bacq2_1 <- glm(ln.HNF_q2 ~ ln.Bac_q2 + HNF_Amntd_select, data = HNF_Bac_A)
HNFq2_Bacq2_2 <- glm(ln.HNF_q2 ~ ln.Bac_q2 * HNF_Amntd_select + Bac_disp + HNF_disp, data = HNF_Bac_A)
anova(HNFq2_Bacq2_0, HNFq2_Bacq2_1, HNFq2_Bacq2_2, test = "F")
summary(HNFq2_Bacq2_2)
...
p_ADiv_Select <- HNF_Bac_A %>% 
  select(ln.Bac_q2, ln.HNF_q2, Bac_Amntd_select, HNF_Amntd_select) %>%
  gather(key = "community", value = "Selection", -c(ln.Bac_q2, ln.HNF_q2)) %>%
  ggplot(aes(x = ln.Bac_q2, y = ln.HNF_q2, color = Selection)) + 
    geom_point(size = 3) + 
    geom_smooth(formula = y ~ x, method = "lm", se = TRUE) + 
    geom_smooth(method = mgcv::gam, formula = y ~ s(x), se = TRUE, color = "red", linetype = "dotted") + 
...
p_ADiv_Select
ggsave(p_ADiv_Select, file = "D:/Research/PdPy_Div_Results/Figs/PR2_2/p_ADiv_HNFq2_Bacq2_Amntd.png",
       dpi = 600, width = 34, height = 28, units = "cm")
##### add selection and dispersal #####
###############################################################################################
##### Simple HNF and Bac alppha diversity relationship  #######################################
...
###############################################################################################
##### Testing hypothesis: HNFq0 -> Bac selection -> Bacq2 & Bacq2 -> Bac selection ->  HNFq0 ##
###############################################################################################
##### HNFq2 -> Bac selection -> Bacq2 ##########
...
Bacq2_BacS.0 <- lm(ln.Bac_q2 ~ Bac_Amntd_select + Bac_disp, data = HNF_Bac_A)
Bacq2_BacS.Cr <- lme(ln.Bac_q2 ~ Bac_Amntd_select + Bac_disp, random = ~ 1 | Cruise, data = HNF_Bac_A)
Bacq2_BacS.Season <- lme(ln.Bac_q2 ~ Bac_Amntd_select + Bac_disp, random = ~ 1 | Season, data = HNF_Bac_A)
Bacq2_BacS.St <- lme(ln.Bac_q2 ~ Bac_Amntd_select + Bac_disp, random = ~ 1 | Station, data = HNF_Bac_A)
AIC(Bacq2_BacS.0, Bacq2_BacS.Cr, Bacq2_BacS.Season, Bacq2_BacS.St)
summary(Bacq2_BacS.0)
summary(gam(ln.Bac_q2 ~ s(Bac_Amntd_select, bs = "ts"), data = HNF_Bac_A))
anova(Bacq2_BacS.0, gam(ln.Bac_q2 ~ s(Bac_Amntd_select, bs = "ts"), data = HNF_Bac_A))
### plotting
p_HNFq2_BacSelect <- HNF_Bac_A %>% 
  select(ln.Bac_q2, ln.HNF_q2, Bac_Amntd_select, HNF_Amntd_select, Cruise) %>%
...
p_BacSelect_Bacq2 <- HNF_Bac_A %>% 
  select(ln.Bac_q2, ln.HNF_q2, Bac_Amntd_select, HNF_Amntd_select, Cruise) %>%
  ggplot(aes(x = Bac_Amntd_select, y = ln.Bac_q2)) + 
  geom_point(aes(color = Cruise), size = 3) + 
  geom_smooth(formula = y ~ x, method = "lm", se = TRUE, linetype = "solid") + 
  geom_smooth(method = mgcv::gam, formula = y ~ s(x), se = TRUE, color = "red", linetype = "solid") + 
  scale_colour_viridis(alpha = 0.7, discrete=TRUE) + 
...
p_HNFq2_BacSelect_Bacq2 <- plot_grid(
  plot_grid(p_HNFq2_BacSelect + theme(legend.position = "none"),
            p_BacSelect_Bacq2 + theme(legend.position = "none")),
  legend, rel_widths = c(3, .4))
p_HNFq2_BacSelect_Bacq2
ggsave(p_HNFq2_BacSelect_Bacq2, file = "D:/Research/PdPy_Div_Results/Figs/PR2_2/p_HNFq2_BacAmntd_Bacq2.png",
       dpi = 600, width = 56, height = 28, units = "cm")
##### HNFq2 -> Bac selection -> Bacq2 ##########

##### Bacq2 -> HNF selection ->  HNFq2 ##########
### linear model testing
HNFS_Bacq2.0 <- lm(HNF_Amntd_select ~ ln.Bac_q2, data = HNF_Bac_A)
HNFS_Bacq2.Cr <- lme(HNF_Amntd_select ~ ln.Bac_q2, random = ~ 1 | Cruise, data = HNF_Bac_A)
HNFS_Bacq2.Season <- lme(HNF_Amntd_select ~ ln.Bac_q2, random = ~ 1 | Season, data = HNF_Bac_A)
HNFS_Bacq2.St <- lme(HNF_Amntd_select ~ ln.Bac_q2, random = ~ 1 | Station, data = HNF_Bac_A)
AIC(HNFS_Bacq2.0, HNFS_Bacq2.Cr, HNFS_Bacq2.Season, HNFS_Bacq2.St)
summary(HNFS_Bacq2.0)
summary(gam(HNF_Amntd_select ~ s(ln.Bac_q2, bs = "ts"), data = HNF_Bac_A))
...
### plotting
p_Bacq2_HNFSelect <- HNF_Bac_A %>% 
  select(ln.Bac_q2, ln.HNF_q0, Bac_Amntd_select, HNF_Amntd_select, Cruise) %>%
  ggplot(aes(x = ln.Bac_q2, y = HNF_Amntd_select)) + 
  geom_point(aes(color = Cruise), size = 3) + 
...
legend <- get_legend(
  p_Bacq2_HNFSelect + theme(legend.box.margin = margin(0, 0, 0, 12))
)
p_Select_Bacq2_HNFq2 <- plot_grid(
  plot_grid(p_Bacq2_HNFSelect + theme(legend.position="none"),
            p_HNFSelect_HNFq2 + theme(legend.position="none")),
  legend, rel_widths = c(3, .4))
p_Select_Bacq2_HNFq2
ggsave(p_Select_Bacq2_HNFq2, file = "D:/Research/PdPy_Div_Results/Figs/PR2_2/p_Bacq2_HNFAmntd_HNFq2.png",
       dpi = 600, width = 56, height = 28, units = "cm")
##### Bacq2 -> HNF selection ->  HNFq0 ##########

##### Path model analysis : Bac_q2 vs HNF_q0 ##########
##### Step 1: no random effects #####
Bacq2_HNFq2_mod1.0 <- '
  # regressions
    ln.Bac_q2 ~ Bac_Amntd_select + Bac_disp 
    ln.HNF_q2 ~ HNF_Amntd_select + HNF_disp
    Bac_Amntd_select ~ ln.HNF_q2
    HNF_Amntd_select ~ ln.Bac_q2
...
Bacq2_HNFq2_mod1.1 <- '
  # regressions
    ...
    Bac_Amntd_select ~ ln.HNF_q2
    HNF_Amntd_select ~ ln.Bac_q2
    ln.Bac_Biom ~ ln.Bac_q2 + ln.HNF_q2
    ln.HNF_Biom ~ ln.HNF_q2 + ln.Bac_q2
...
Bacq2_HNFq2_mod1.3 <- '
  # regressions
    ln.Bac_q2 ~ Bac_Amntd_select + Bac_disp + ln.HNF_Biom
    ln.HNF_q2 ~ HNF_Amntd_select + HNF_disp + ln.Bac_Biom
    Bac_Amntd_select ~ ln.HNF_q2
    HNF_Amntd_select ~ ln.Bac_q2
    ln.Bac_Biom ~ ln.Bac_q2
...
Bacq2_HNFq2_mod1.5 <- '
  # regressions
    ...
    HNF_Amntd_select ~ ln.Bac_q2
    ln.Bac_Biom ~ ln.Bac_q2 + ln.HNF_q2
    ln.HNF_Biom ~ ln.HNF_q2 + ln.Bac_q2 + ln.Bac_Biom
...
Bacq2_HNFq2_mod1.9 <- '
  # regressions
    ...
    Bac_Amntd_select ~ ln.HNF_q2
    HNF_Amntd_select ~ ln.Bac_q2
    ln.Bac_Biom ~ ln.Bac_q2 + ln.HNF_q2 + ln.HNF_Biom
    ln.HNF_Biom ~ ln.HNF_q2 + ln.Bac_q2
...
Bacq2_HNFq2_mod1.10 <- '
  # regressions
    ...
    Bac_Amntd_select ~ ln.HNF_q2
    HNF_Amntd_select ~ ln.Bac_q2
    ln.Bac_Biom ~ ln.Bac_q2 + ln.HNF_Biom
    ln.HNF_Biom ~ ln.HNF_q2 + ln.Bac_q2
...
Bacq2_HNFq2_mod1.12 <- '
  ...
  # correlated error
    ln.Bac_q2 ~~ ln.HNF_q2
'
Bacq2_HNFq2_lavaan1.0 <- sem(Bacq2_HNFq2_mod1.0, data = HNF_Bac_A)#, se = "bootstrap")
Bacq2_HNFq2_lavaan1.1 <- sem(Bacq2_HNFq2_mod1.1, data = HNF_Bac_A)
Bacq2_HNFq2_lavaan1.2 <- sem(Bacq2_HNFq2_mod1.2, data = HNF_Bac_A)
Bacq2_HNFq2_lavaan1.3 <- sem(Bacq2_HNFq2_mod1.3, data = HNF_Bac_A)
Bacq2_HNFq2_lavaan1.4 <- sem(Bacq2_HNFq2_mod1.4, data = HNF_Bac_A)
Bacq2_HNFq2_lavaan1.5 <- sem(Bacq2_HNFq2_mod1.5, data = HNF_Bac_A)
Bacq2_HNFq2_lavaan1.6 <- sem(Bacq2_HNFq2_mod1.6, data = HNF_Bac_A)
Bacq2_HNFq2_lavaan1.7 <- sem(Bacq2_HNFq2_mod1.7, data = HNF_Bac_A)
Bacq2_HNFq2_lavaan1.8 <- sem(Bacq2_HNFq2_mod1.8, data = HNF_Bac_A)
Bacq2_HNFq2_lavaan1.9 <- sem(Bacq2_HNFq2_mod1.9, data = HNF_Bac_A)
Bacq2_HNFq2_lavaan1.10 <- sem(Bacq2_HNFq2_mod1.10, data = HNF_Bac_A)
Bacq2_HNFq2_lavaan1.11 <- sem(Bacq2_HNFq2_mod1.11, data = HNF_Bac_A)
Bacq2_HNFq2_lavaan1.12 <- sem(Bacq2_HNFq2_mod1.12, data = HNF_Bac_A)

AICstep1 <- AIC(Bacq2_HNFq2_lavaan1.0, Bacq2_HNFq2_lavaan1.1, Bacq2_HNFq2_lavaan1.2, Bacq2_HNFq2_lavaan1.3,
                Bacq2_HNFq2_lavaan1.4, Bacq2_HNFq2_lavaan1.5, Bacq2_HNFq2_lavaan1.6, Bacq2_HNFq2_lavaan1.7,
                Bacq2_HNFq2_lavaan1.8, Bacq2_HNFq2_lavaan1.9, Bacq2_HNFq2_lavaan1.10, Bacq2_HNFq2_lavaan1.11,
                Bacq2_HNFq2_lavaan1.12)
...
'
Bacq2_HNFq2_lavaan1.10 <- sem(Bacq2_HNFq2_mod1.10, data = HNF_Bac_A)
summary(Bacq2_HNFq2_lavaan1.10, fit.measures = TRUE)
summary(Bacq2_HNFq2_lavaan1.0, fit.measures = TRUE)
##### Step 1: no random effects #####
##### Step 2 : include grouping variables (random effects) and environmental variables #####
Bacq2_HNFq0_psem2.0 <- psem(
  lm(ln.Bac_q2 ~ Bac_select + Bac_disp + ln.HNF_Biom, # + ln.Temp + ln.Sal + ln.PAR + ln.DIN + ln.PO3 + ln.Chla, 
      data = HNF_Bac_A),
  ...
  lm(Bac_select ~ ln.HNF_q0 + ln.Temp + ln.Sal + ln.PAR + ln.DIN + ln.PO3 + ln.Chla,
      data = HNF_Bac_A),
  # lm(HNF_select ~ ln.Bac_q2 + ln.Temp + ln.Sal + ln.PAR + ln.DIN + ln.PO3 + ln.Chla,
  #     data = HNF_Bac_A),

  # lm(ln.Bac_Biom ~ ln.Bac_q2 + ln.HNF_Biom + ln.Temp + ln.Sal + ln.PAR + ln.DIN + ln.PO3 + ln.Chla,
  #     data = HNF_Bac_A),
...
summary(Bacq2_HNFq0_psem2.0)
Bacq2_HNFq0_psem2.Cr <- psem(
  lme(ln.Bac_q2 ~ Bac_select + Bac_disp + ln.HNF_Biom, # + ln.Temp + ln.Sal + ln.PAR + ln.DIN + ln.PO3 + ln.Chla, 
      random = ~ 1 | Cruise, data = HNF_Bac_A),
  ...
  lme(Bac_select ~ ln.HNF_q0 + ln.Temp + ln.Sal + ln.PAR + ln.DIN + ln.PO3 + ln.Chla, 
      random = ~ 1 | Cruise, data = HNF_Bac_A),
  # lme(HNF_select ~ ln.Bac_q2 + ln.Temp + ln.Sal + ln.PAR + ln.DIN + ln.PO3 + ln.Chla, 
  #     random = ~ 1 | Cruise, data = HNF_Bac_A),
  # 
  # lme(ln.Bac_Biom ~ ln.Bac_q2 + ln.HNF_Biom + ln.Temp + ln.Sal + ln.PAR + ln.DIN + ln.PO3 + ln.Chla, 
  #    random = ~ 1 | Cruise, data = HNF_Bac_A),
...
summary(Bacq2_HNFq0_psem2.Cr)
Bacq2_HNFq0_psem2.Season <- psem(
  lme(ln.Bac_q2 ~ Bac_select + Bac_disp + ln.HNF_Biom, #  + ln.Temp + ln.Sal + ln.PAR + ln.DIN + ln.PO3 + ln.Chla, 
...
summary(Bacq2_HNFq0_psem2.Season)
Bacq2_HNFq0_psem2.St <- psem(
  lme(ln.Bac_q2 ~ Bac_select + Bac_disp + ln.HNF_Biom, #  + ln.Temp + ln.Sal + ln.PAR + ln.DIN + ln.PO3 + ln.Chla, 
...
summary(Bacq2_HNFq0_psem2.St)

anova(Bacq2_HNFq0_psem2.0, Bacq2_HNFq0_psem2.Cr, Bacq2_HNFq0_psem2.Season, Bacq2_HNFq0_psem2.St, test = "F")
anova(Bacq2_HNFq0_psem2.0, Bacq2_HNFq0_psem2.Cr)
summary(Bacq2_HNFq0_psem2.Cr, fit.measures = TRUE)
##### Path model analysis : Bac_q2 vs HNF_q0 ##########
###############################################################################################
##### Testing hypothesis: HNFq0 -> Bac selection -> Bacq2 & Bacq2 -> Bac selection ->  HNFq0 ##
###############################################################################################
