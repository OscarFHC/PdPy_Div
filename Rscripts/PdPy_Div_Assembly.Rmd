...
```{r, Bacteria phylogenetic signal}
...
```{r, Nanoflagellate phylogenetic signal}
...
BacPhylo_null <- abind(as.matrix(comdistnt(Bac_comm, cophenetic(Bac_phylo))), 
                       replicate(nsim, comdistnt.shuffle(Bac_comm, Bac_phylo)))
write.table(as.matrix(BacPhylo_null), file = "D:/Research/PdPy_Div/Results/Bac_MNTD_null.csv", 
            sep = ",", row.names = FALSE, col.names = FALSE)
BacPhylo_null <- array(dim = c(nrow(Bac_comm), nrow(Bac_comm), nsim + 1), 
                       as.matrix(read.table(file = "D:/Research/PdPy_Div/Results/Bac_MNTD_null.csv", sep = ",")))

NF_null <- abind(as.matrix(comdistnt(NF_comm, cophenetic(NF_phylo))), 
                 replicate(999, comdistnt.shuffle(NF_comm, NF_phylo)))
write.table(as.matrix(NF_null), file = "D:/Research/PdPy_Div/Results/NF_MNTD_null.csv", 
            sep = ",", row.names = FALSE, col.names = FALSE)
NF_null <- array(dim = c(nrow(NF_comm), nrow(NF_comm), nsim + 1), 
                  as.matrix(read.table(file = "D:/Research/PdPy_Div/Results/NF_MNTD_null.csv", sep = ",")))

Bac_ses <- (Bac_null[,,1] - apply(Bac_null[,,-1], c(1, 2), mean)) / apply(Bac_null[,,-1], c(1, 2), sd)
NF_ses <- (NF_null[,,1] - apply(NF_null[,,-1], c(1, 2), mean)) / apply(NF_null[,,-1], c(1, 2), sd)

Bac_rank <- array(dim = dim(Bac_null), t(apply(apply(Bac_null, c(1, 2), rank), 3, t)))
NF_rank <- array(dim = dim(NF_null), t(apply(apply(NF_null, c(1, 2), rank), 3, t)))
```
