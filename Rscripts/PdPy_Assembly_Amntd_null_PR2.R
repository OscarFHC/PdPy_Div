...
test[[1000]] <- as.matrix(mntd(Bac_comm, cophenetic(tipShuffle(Bac_phylo)), abundance.weighted = TRUE))
...
NFPhylo_null <- data.frame(matrix(unlist(test), ncol = length(test), byrow = FALSE)) %>%
  ...
  rename(obs = X1000)
write.table(NFPhylo_null, file = "D:/Research/PdPy_Div_Results/NF_MPD_null_PR2.csv",
            sep = ",", col.names = TRUE, row.names = FALSE)
stopCluster(cl)
##### Nanoflagellate phylogenetic turnover ##########################################
