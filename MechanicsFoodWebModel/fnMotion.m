...
%%%% READ ME %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% The following code defines 12 functions used in the calculation of motion speeds and costs 
% fnMotion
% fnVelocity 
% fnVelocityStiff
% fnHandlingMotion
...
% 2. The vertical distance travelled is equal to body length (upwards and downwards)
...
function [Output] = fnHandlingMotion(x,p)
  ...
  DynamicViscosity=p(8);
  TimeStep=p(9);
  % i_Switch=p(10); % unused within fnHandlingMotion function
  ...
  if flag==0
      if activedistancetravelled > BodyLength
          index=find(totaldistance >= BodyLength);
          activetimeupwards=t1(index(1));
  ...
  if flag==0
      ...
      if totaldistance(end) > totalascendingdistance
          ...
          % Extrapolate when distance travelled = total ascending distance travelled (knowing that the individual body has reached its terminal velocity)
          remainingdistance=BodyLength-totaldistance(end);
          remainingtime=remainingdistance/res3(end);
          passivetimedownwards=t3(end)+remainingtime;
      end
  ...
  if flag==0
      ...
      % The predator cannot lift itself and the prey
      Output=NaN;
  end
